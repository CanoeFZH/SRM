...
class CatsOnTheLineDiv1
{
    public:
        int getNumber(vector <int> position, vector <int> count, int time) {
            ...
            for (int i = 0; i < n; i++) {
                p.push_back(make_pair(position[i], count[i]));
            }
            sort(p.begin(), p.end());
            
            const int INF = 1 << 28;
            int occupiedLeftPosition = -INF, occupiedRightPosition = -INF, ret = 0;
            for (int i = 0; i < n; i++) {
                int L = p[i].first - time, R = p[i].first + time;
                if (L <= occupiedRightPosition && occupiedRightPosition <= R) {
                    continue;
                }
                L = max(L, occupiedLeftPosition);
                if (R - L + 1 >= p[i].second) {
                    occupiedLeftPosition = L + p[i].second;
                } else {
                    ret ++;
                    occupiedRightPosition = R;
                }
            }
            return ret;
        }
