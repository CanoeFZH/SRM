...
class PairGame
{
    public:
        ...
        void gao(int a, int b, int flag) {
            pairCnt[make_pair(a, b)] |= flag;
            if (a == b) {
                return;
            } else if (a > b) {
                if ((pairCnt[make_pair(a - b, b)] & flag) == 0){
                    gao(a - b, b, flag);
                }
            } else {
                if ((pairCnt[make_pair(a, b - a)] & flag) == 0){
                    gao(a, b - a, flag);
                }
            }
        }
        int maxSum(int a, int b, int c, int d) {
            ...
            for (auto &x: pairCnt) {
                if (x.second == 3) {
                    ret = max(ret, x.first.first + x.first.second);
                }
            }
            return ret;
        }
